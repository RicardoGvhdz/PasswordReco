...
const users = {
    "Ricardo": "Segur0Ricardo",        // Usuario estándar
    "RicardoAdm": "AdminSeguro123"      // Usuario administrador
};
...
document.getElementById('loginForm').addEventListener('submit', function (e) {
    ...
    if (users[username] && users[username] === password) {
        ...
        if (username === "RicardoAdm") {
            window.location.href = "hub.html";  // Redirección a la vista de administrador
        } else {
            window.location.href = "Events.html";  // Redirección a la vista de usuario estándar
        }
    } else {
...
document.getElementById('loginForm').addEventListener('submit', function (e) {
    ...
    if (userData && userData.password === password) {
        ...
        if (username === "RicardoAdm") {
            window.location.href = "hub.html"; // Administrador
        } else {
            window.location.href = "Events.html"; // Usuario estándar
        }
    } else {
        message.style.color = 'red';
        message.innerText = 'Usuario o contraseña incorrectos.';
